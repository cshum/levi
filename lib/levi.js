...
function put (ctx, next) {
  ...
  var tfInit = {}
  var fieldsOpts = ctx.options.fields || {'*': true}

  if (Array.isArray(fieldsOpts)) {
    var arr = fieldsOpts
    fieldsOpts = {}
    arr.forEach(function (field) {
      fieldsOpts[field] = true
    })
  }
  var fields = []
  ...
  return H(function (push, next) {
    ...
    self.pipeline(q, function (err, tokens) {
      ...
      for (var token in counts) {
        // calculate tf for query
        query[token] = counts[token] / total
        // cache initial doc count for query
        self.docCount.get(token, function (e, nt) {
          count++
          if (nt) nts[token] = nt
